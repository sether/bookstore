...
// Listing:	Chapter 21, Listing 21.3
import java.awt.*;
...
public class CustomerGUI extends JFrame implements ActionListener {
	...
	private String fieldNames[] = {"Customer Id", "First Name", "Email",
				"Last Name", "City", "Country", "Record No."};
	private String errorMessage;

	public CustomerGUI() {					// Constructor for GUI
		...
		Connection connNWind = Connect.openDatabase("jdbc:odbc:nwind");  // 'nwind' is ODBC
		if (connNWind == null) {
		...
		btnSave.setEnabled(false);	// User can add or update, but not save
 	...
 	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnNext) {
			...
			statement.moveCursor(StatementSet.CURSOR_FIRST);
			this.custTableToFrame(gui);			// Display first customer
		}
			...
			int response;
			response = JOptionPane.showConfirmDialog(null, "Add customer - are you sure", "",
			...
			btnAdd.setEnabled(false);			// User can only save
			btnUpdate.setEnabled(false);
			}
		}
		else if (e.getSource() == btnSave) {
			if (this.addCustomer(gui)) {
				JOptionPane.showMessageDialog(null, "Customer added - to end of record set");
				btnSave.setEnabled(false);
				btnAdd.setEnabled(true);		// User can now add or update
				btnUpdate.setEnabled(true);
			}
			else
	...
	final static int CUSTID_LENGTH = 5;	// CustomerID field in cust table MUST be 5 chars
	boolean addCustomer(GenericPanel gui) { // Add a customer record at the 'insert' row
		boolean success = true;
		String custId = "";

		custId = gui.getTextField(0);	
		if (custId.length() != CUSTID_LENGTH) {  // Validate the primary key
			errorMessage = "CustomerId field must be exactly 5 characters";
			return false;
		}
		try {
			rsCustomer.moveToInsertRow();
			rsCustomer.updateString("CustomerID", custId);
			...
			totalRows++;
			gui.setTextField(5, Integer.toString(totalRows) + " / " +
	...
	void custFrameToTable(GenericPanel gui) {  // Copy frame fields to table fields
		try {   // Only update the primary key field (0) when adding
			rsCustomer.updateString("FirstName", gui.getTextField(1));
			rsCustomer.updateString("LastName", gui.getTextField(2));
			rsCustomer.updateString("Email", gui.getTextField(3));
			rsCustomer.updateString("City", gui.getTextField(4));
			rsCustomer.updateString("Country", gui.getTextField(5));
		}
		catch(SQLException ex) {
	...
	void custTableToFrame(GenericPanel gui) { // Copy table fields to frame fields
		try {
			gui.setTextField(0, rsCustomer.getString("CustID"));
			gui.setTextField(1, rsCustomer.getString("FirstName"));
			gui.setTextField(2, rsCustomer.getString("LastName"));
			gui.setTextField(3, rsCustomer.getString("Email"));
			gui.setTextField(4, rsCustomer.getString("City"));
			gui.setTextField(5, rsCustomer.getString("Country"));
			gui.setTextField(6, Integer.toString(rsCustomer.getRow()) + " / " +
						Integer.toString(totalRows));
		}
		catch(SQLException ex) {
			System.err.println(ex.getMessage());
	...
	void getTotalRows() {				// Get number of rows in set
		try {
			if (rsCustomer.last())
